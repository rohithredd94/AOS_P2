...
public class ChandyLamport {
	...
	public static void sendMarkerMessage(ProjectMain mainObj, int channelNo){
		...
		synchronized(mainObj){
			if(mainObj.is_blue){
				System.out.println("Received first Marker message from node and color is blue, " + "will be changed to red  "+channelNo);
...
//				new writeToOutputThread(mainObj).start();
				...
				for(int i : mainObj.neighbors){
					...
					m.nodeId = mainObj.id;
					ObjectOutputStream oos = mainObj.output_stream.get(i);
					try {
						oos.writeObject(m);
					} catch (IOException e) {
						e.printStackTrace();
				...
				if((mainObj.neighbors.length == 1) && (mainObj.id!=0)){
					...
					mainObj.myState.in_transit_msgs = mainObj.in_transit_msgs;
					mainObj.is_blue = true;
					mainObj.logging = 0;
					// Send channel state to parent 
					ObjectOutputStream oos = mainObj.output_stream.get(parent);
			...
			//If color of the process is red and a marker message is received on this channel
				...
				while(i<mainObj.neighbors.length && mainObj.rec_marker.get(mainObj.neighbors[i]) == true){
...
//					}
				...
				if(i == mainObj.neighbors.length &&  mainObj.id == 0){
					...
					mainObj.myState.in_transit_msgs = mainObj.in_transit_msgs;
					mainObj.state_messages.put(mainObj.id, mainObj.myState);
					mainObj.is_blue = true;
					mainObj.logging = 0;
				}
...
//				
	...
	public static boolean processstate_messages(ProjectMain mainObj, StateMsg msg) throws InterruptedException {
		int i=0,j=0,k=0;
		synchronized(mainObj){
			...
			if(i == mainObj.nodesInGraph.length){
				...
				for(j=0;j<mainObj.state_messages.size();j++){
					...
					if(mainObj.state_messages.get(j).active == true){
				...
				if(j == mainObj.numOfNodes){
					...
					for(k=0;k<mainObj.numOfNodes;k++){
						...
						StateMsg value = mainObj.state_messages.get(k);
...
//								for(ApplicationMsg m:g)
	...
	public static void logMessage(int channelNo,ApplicationMsg m, ProjectMain mainObj) {
		synchronized(mainObj){
			...
			if(!(mainObj.in_transit_msgs.get(channelNo).isEmpty()) && mainObj.rec_marker.get(channelNo) != true){
				mainObj.in_transit_msgs.get(channelNo).add(m);
			}
			// or create a list and add the message into it
			else if((mainObj.in_transit_msgs.get(channelNo).isEmpty()) && mainObj.rec_marker.get(channelNo) != true){
				ArrayList<ApplicationMsg> msgs = mainObj.in_transit_msgs.get(channelNo);
				msgs.add(m);
				mainObj.in_transit_msgs.put(channelNo, msgs);
			}
		}
	}
	...
	public static void sendFinishMsg(ProjectMain mainObj) {
		synchronized(mainObj){
			...
			for(int s : mainObj.neighbors){
				...
				ObjectOutputStream oos = mainObj.output_stream.get(s);
				try {
					oos.writeObject(m);
				} catch (IOException e) {
					e.printStackTrace();
